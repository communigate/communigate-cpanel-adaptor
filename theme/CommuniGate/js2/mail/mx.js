...
var after_show_module = function(id) {
	...
	if (type == "edit") {
		...
		CHANGE_VALID["content_changed"].add("old_destination_" + id, function() { return content_changed(id); }, LANG.must_change_before_edit);
		CHANGE_VALID["content_changed"].attach();
		
		CPANEL.validate.attach_to_form("confirm_edit_" + id, CHANGE_VALID, function() { edit_mx_record(id) });
        ...
        success : function(o) {
			...
			if (data.cpanelresult.error) {
                ...
                if ( data.cpanelresult.data[0].checkmx.warnings &&  data.cpanelresult.data[0].checkmx.warnings.length ) {
                    ...
                    CPANEL.widgets.status_bar("add_new_record_status_bar", "warning", LANG.MX_added_record, preference + " &rarr; " + exchanger + '<br />' + warnings_html, { noCountdown: true });	
                } else {
        ...
        success : function(o) {
			try {
				...
				if (data.cpanelresult.error) {
                    ...
                    if ( data.cpanelresult.data[0].checkmx.warnings &&  data.cpanelresult.data[0].checkmx.warnings.length ) {
                        var warnings_html = build_warnings_html(data.cpanelresult.data[0].checkmx.warnings);
                        CPANEL.widgets.status_bar("status_bar_" + id, "warning", LANG.MX_changed_record, warnings_html, { noCountdown: true });	
                    } else {
