...
var after_show_module = function(o) {
    ...
    if (o.action == 'rename') {
	$("#rename_" + o.index).click(function() {
		rename_confirm(o);
	    });
    }
    if (o.action == 'settings') {
	$("#settings_" + o.index).click(function() {
...
var after_hide_module = function(o) {
    ...
    if (o.action == 'settings') {
	$("#settings_" + o.index).unbind("click");
	$("#scc_msg_wrap").hide();
	$("#err_msg_wrap").hide();
    }
};
...
var get_group_settings = function(obj) {
    ...
    var success = function(o) {
	    ...
	    html += settings_template({
			...
			EmailDisabled: (data.EmailDisabled == 'NO') ? 'checked' : '',
...
var set_group_settings = function(obj) {
    ...
    var api2_call = {
        cpanel_jsonapi_version: 2,
        cpanel_jsonapi_module: "CommuniGate",
        cpanel_jsonapi_func: "SetDepartmentSettings",
    	email : obj.group,
    	Expand : (selected.Expand) ? selected.Expand : '',
    	RealName : $("#RealName").val(),
	EmailDisabled: (selected.EmailDisabled) ? selected.EmailDisabled : ''
    };
