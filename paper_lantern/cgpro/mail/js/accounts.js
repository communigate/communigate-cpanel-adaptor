...
var get_server = function () {
	...
	var success = function (res) {
		var data = JSON.parse(res);
		if (data.cpanelresult.event.result) {
			server = data.cpanelresult.data[0];
		}
	}
	get_accounts();
// send the request
...
function load_delete_template(item) {
	...
	if ($("#show_template_" + account_id).find("div.delete_template").length < 1) {
		var delete_tmpl_html = new EJS({url: 'delete_template.ejs'}).render({
			"account": account,
		...
		$("#show_template_" + account_id).hide().html(delete_tmpl_html).promise().done(function () {
			setTimeout(function () {
				...
				item.parent().parent().next('tr').addClass('extended');
			}, 1);
		});
		$("#cancel_btn_" + account_id).click(function () {
			$("#show_template_" + account_id).slideUp("slow", function () {
				item.parent().parent().next('tr').removeClass('extended');
			}).html("");
		});
...
function load_details_template(item) {
	...
	if ($("#show_template_" + account_id).find("div.details_template").length < 1) {
		var details_tmpl_html = new EJS({url: 'details_template.ejs'}).render({
			"account": account,
			"account_id": account_id
		});
...
function load_password_template(item) {
	...
	if ($("#show_template_" + account_id).find("div.password_template").length < 1) {
		var pass_tmpl_html = new EJS({url: 'password_template.ejs'}).render({
			"account": account,
			"account_id": account_id
		});
...
var load_avatar_template = function (item) {
	...
	if ($("#show_template_" + account_id).find("div.avatar_template").length < 1) {
		var change_avatar_tmpl_html = new EJS({url: 'avatar_template.ejs'}).render({
			"account": account,
			"account_id": account_id
		});
		...
		$("#cancel_btn_" + account_id).click(function () {
			$("#show_template_" + account_id).slideUp("slow", function () {
				item.parent().parent().parent().next('tr').removeClass('extended');
			}).html("");
		});
...
function load_active_sync_template(account_id, data, element) {
	var account = accounts[account_id];
	console.log(server);
	var active_sync_tmpl_html = new EJS({url: 'active_sync_template.ejs'}).render({
		"account": account,
		"account_id": account_id,
		"active_sync_data": data,
		"server": server
	});
	...
	$("#cancel_btn_" + account_id).click(function () {
		$("#show_template_" + account_id).slideUp("slow", function () {
			element.parent().parent().parent().parent().next('tr').removeClass('extended');
		}).html("");
	});
}
...
function load_active_sync_data(item, element) {
	...
	if ($("#show_template_" + item).find("div.active_sync_template").length < 1) {
		...
		$("#status_bar_" + item).hide().promise().done(function () {
			...
			setTimeout(function () {
				$("#status_bar_" + item).slideDown("slow", function () {
					$(this).html(CPANEL.icons.ajax + " Loading...")
				});
			}, 1);
		});
		var api2_call = {
		...
		var success = function (res) {
			$("#status_bar_" + item).slideUp("slow", function () {
				$(this).html("")
			});
			var data = JSON.parse(res);
		...
		$("#show_template_" + item).slideUp("slow", function () {
			$(this).html("");
			element.parent().parent().parent().parent().next('tr').removeClass('extended');
		});
	}
...
var change_details = function (account_id) {
	...
	var work_days = $('input:checkbox[name=work_days]:checked').map(function () {
		return this.value;
	}).get();
	for (var i = 0; i < work_days.length; i++) {
		switch (work_days[i]) {
			case "Sun":
				api2_call['WorkDays'] = work_days[i];
				break;
			case "Mon":
				api2_call['WorkDays-0'] = work_days[i];
				break;
			case "Tue":
				api2_call['WorkDays-1'] = work_days[i];
				break;
			case "Wed":
				api2_call['WorkDays-2'] = work_days[i];
				break;
			case "Thu":
				api2_call['WorkDays-3'] = work_days[i];
				break;
			case "Fri":
				api2_call['WorkDays-4'] = work_days[i];
				break;
			case "Sat":
				api2_call['WorkDays-5'] = work_days[i];
				break;
		}
	}
