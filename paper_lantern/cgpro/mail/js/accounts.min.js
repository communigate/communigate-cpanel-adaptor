document.addEventListener("DOMContentLoaded", function() {
    require(["frameworksBuild"], function() {
        require(["jquery"], function($) {
            ...
            CPANEL.password.create_password = function (options){
                var length;
                ...
                if(options.uppercase==true) {
                    rnum=Math.floor(Math.random()*uppercase.length);
                    password+=uppercase.substring(rnum,rnum+1);
                } else {
                    rnum=Math.floor(Math.random()*chars.length);
                    password+=chars.substring(rnum,rnum+1);
                }
                if(options.lowercase==true) {
                    rnum=Math.floor(Math.random()*lowercase.length);
                    password+=lowercase.substring(rnum,rnum+1);
                } else {
                    rnum=Math.floor(Math.random()*chars.length);
                    password+=chars.substring(rnum,rnum+1);
                }
            ...
            var get_accounts = function () {
                ...
                var success = function (res) {
                    ...
                    if (data.cpanelresult.event.result) {
                        ...
                        var html = new EJS({url: 'accounts_table.ejs'}).render({"accounts": accounts, "classes": classes});
            ...
            var initialize_new_email_validators = function () {
                ...
                CHANGE_PASSWORD1_VALIDATION.add(password1, 'no_chars(%input%, " ")', "Password should not contain spaces!");
                CHANGE_PASSWORD1_VALIDATION.attach();

                CHANGE_PASSWORD2_VALIDATION = new CPANEL.validate.validator("Passwords do not match");
                CHANGE_PASSWORD2_VALIDATION.add(password2, "equals('" + password1 + "', '" + password2 + "')", "Passwords do not match!");
            ...
            function load_plan_template(item) {
                ...
                if ($("#show_template_" + account_id).find("div.plan_template").length < 1) {
                    var plan_tmpl_html = new EJS({url: 'plan_template.ejs'}).render({"account": account, "account_id": account_id, "classes": classes});
